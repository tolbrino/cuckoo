...
struct edgetrimmer {
  ...
  ~edgetrimmer() {
    ...
    if checkCudaErrors_B(cudaFree(dt));
    cudaDeviceReset();
  }
  u32 trim() {
    cudaMemcpy(dt, this, sizeof(edgetrimmer), cudaMemcpyHostToDevice);
    ...
    if checkCudaErrors(cudaEventCreate(&startall)) return cuckooErr;
    if checkCudaErrors(cudaEventCreate(&stopall)) return cuckooErr;
...
int main(int argc, char **argv) {
  ...
  cudaDeviceProp prop;
  if checkCudaErrors_B(cudaGetDeviceProperties(&prop, device)) return NULL;
